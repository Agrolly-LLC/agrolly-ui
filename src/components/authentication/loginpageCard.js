      ...
      .then((response) => {
        ...
        if (response.data.result === "successful") {
          ...
          localStorage.setItem(
            "passwordtoken",
            JSON.stringify(this.state.userpassword)
          );
          // console.log(response.data)
          window.location = "/farmer/cropSell";
        } else {
          this.setState({
            errorMessage: "email or password is worng please try again",
          });
        }
      })
      .catch((err) => {
    ...
    return (
      <div className={style.backgroundLogin}>
        <div className={style.formcontainer}>
          ...
          <h6>
            <a style={{ color: "blue" }} href="/farmer/forgetpassword">
              forget password
            </a>
          </h6>
        </div>
...
const UserLogin = withTranslation()(UserLoginCard);
